...
class MeetingController < ApplicationController
  def home
    ...
    response = HTTParty.get(getRecordingLink, verify: true, timeout: 6000)
    recordingsXML = Nokogiri::XML(response.body).xpath('//recording')
    @recordings = []
    recordingsXML.each do |recording|
  ...
  def create
    ...
    response = HTTParty.get(createLink, verify: true, timeout: 6000)
  ...
  def delete
    ...
    response = HTTParty.get(deleteLink, verify: true, timeout: 6000)
    redirect_to '/'
  end

  def join
    # Join the meeting
    if params.key?(:meetingID)
